...
class Profile extends CI_Controller 
{
	...
	public function delete_image($id, $hash) 
	{
		...
		if(!empty($image->file_url)) {
		 ...
		 $this->image_model->decrease_album_count($album->ID);
	...
	public function friends_page($userid) 
	{
		...
		foreach($friends->result() as $r) {
			if($user->ID == $this->user->info->ID) {
				$options = '<a href="'.site_url("profile/deadd/" . $r->ID . "/" . $this->security->get_csrf_hash()).'" class="btn btn-danger btn-xs">'.lang("ctn_659").'</a>';
			} else {
				$options = "";
			}
			$this->datatables->data[] = array(
